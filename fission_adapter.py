...
class Scheduler:
    ...
    def is_pod_warm(self, fission_fn_name):
    ...
    def find_warm_pod_function_name(self, function_path, lng, power_set):
        for package in power_set:
            fission_fn_name = self.get_function_name(function_path, lng, package)
            print("package:", package, "fission_fn_name:", fission_fn_name)

            if self.is_pod_warm(fission_fn_name):
                return fission_fn_name
        return None
    ...
    def execute_function(self, function_path: str):
        ...
        warm_pod_fn_name = self.find_warm_pod_function_name(function_path, lng, power_set)
        if warm_pod_fn_name:
            # A warm function has been found and tested.
            # Function exits here if found, no need to return anything.
            return
...
if __name__ == "__main__":
    ...
    scheduler.submit_function("pn.py", "python", frozenset({"numpy", "fastapi"}))
